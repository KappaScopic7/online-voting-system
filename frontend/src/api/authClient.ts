...
export async function fetchElectionResult(
    ...
    if (res.status === 403) {
        const text = await res.text();
        throw new Error(text || 'この選挙の結果はまだ閲覧できません。');
    }
...
export async function fetchIdentityStatus(
    ...
    const res = await fetch(`${API_BASE}/api/voters/identity/status?electionId=${electionId}`, {
        headers: { Authorization: `Bearer ${token}` },
    });

    if (res.status === 401 || res.status === 403) throw new Error('unauthorized');
    if (!res.ok) throw new Error((await res.text()) || '本人認証状態の取得に失敗しました');
...
export async function verifyIdentity(
    ...
    const res = await fetch(`${API_BASE}/api/elections/${electionId}/verification`, {
        method: 'POST',
        headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
    ...
    if (!res.ok && res.status !== 204) {
        const text = await res.text();
        throw new Error(text || '本人認証に失敗しました');
    }
}

export async function fetchMyVerification(token: string, electionId: number): Promise<boolean> {
    const res = await fetch(`${API_BASE}/api/elections/${electionId}/verification/me`, {
        method: 'GET',
        headers: {
            Authorization: `Bearer ${token}`,
        },
    ...
    if (!res.ok) {
        const text = await res.text();
        throw new Error(text || '本人認証状態の取得に失敗しました');
    }
